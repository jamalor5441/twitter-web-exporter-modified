    ...
    async init() {
      try {
        ...
        this.db.version(1).stores({
          tweets: 'rest_id,twe_private_fields.created_at,twe_private_fields.updated_at,twe_private_fields.media_count',
          users: 'rest_id,twe_private_fields.created_at,twe_private_fields.updated_at',
          captures: 'id,extension,type,created_at,data_key'
        });
        
        // 版本2：添加更多索引以提升性能
        this.db.version(DB_VERSION).stores({
          tweets: 'rest_id,twe_private_fields.created_at,twe_private_fields.updated_at,twe_private_fields.media_count,core.user_results.result.core.screen_name',
          users: 'rest_id,twe_private_fields.created_at,twe_private_fields.updated_at,core.screen_name',
          captures: 'id,extension,type,created_at,data_key'
        }).upgrade(async (tx) => {
  ...
  return this.db.transaction("rw", this.tweets(), async () => {
    ...
    const data = tweets.map((tweet, index) => {
      ...
      return {
        ...tweet,
        twe_private_fields: {
          created_at: +parseTwitterDateTime(tweet.legacy.created_at), // 解析时间戳
...
async function zipStreamDownload(zipFilename, files, onProgress, rateLimit = 1e3, batchSize = 10) {
  ...
  const DEFAULT_FILENAME_PATTERN = "{screen_name}-{id}-{date}-{time}-【{name}】-{type}-{num}-{full_text}.{ext}";
